...
void cmd_68(uint8_t tx_cmd[2])
{
  ...
  cmd[3] = (uint8_t)(cmd_pec);
	LTC6811_CS_Low_1();
  LTC6811_SPI_Write_Array_1(4,cmd);
	LTC6811_CS_High_1();
}
...
void write_68(uint8_t total_ic , uint8_t tx_cmd[2], uint8_t data[])
{
  ...
  LTC6811_CS_Low_1();
  LTC6811_SPI_Write_Array_1(CMD_LEN, cmd);
  LTC6811_CS_High_1();
  free(cmd);
}
...
int8_t read_68( uint8_t total_ic, uint8_t tx_cmd[2], uint8_t *rx_data)
{
  ...
  LTC6811_CS_Low_1();
  LTC6811_SPI_Write_Read_1(cmd, 4, data, (BYTES_IN_REG*total_ic));         //Read the configuration data of all ICs on the daisy chain into
  LTC6811_CS_High_1();                          //rx_data[] array
...
uint8_t LTC681x_pladc()
{
  ...
  LTC6811_CS_Low_1();
  LTC6811_SPI_Write_Array_1(4,cmd);
	//adc_state = LTC6811_SPI_Read_Byte_1(0xFF);
...
uint32_t LTC681x_pollAdc()
{
  ...
  {
    current_time = LTC6811_SPI_Read_Byte_1(0xff);
    if (current_time>0)
    {
      finished = 1;
    }
                     ...
                     )
{
  ...
  LTC6811_CS_Low_1();
  LTC6811_SPI_Write_Read_1(cmd,4,data,(REG_LEN*total_ic));
  LTC6811_CS_High_1();
...
//Generic function to write 68xx commands. Function calculated PEC for tx_cmd data
void cmd_68_2_2(uint8_t tx_cmd[2])
{
  uint8_t cmd[4];
  uint16_t cmd_pec;
  uint8_t md_bits;
  ...
  cmd[0] = tx_cmd[0];
  cmd[1] =  tx_cmd[1];
  cmd_pec = pec15_calc_2(2, cmd);
  cmd[2] = (uint8_t)(cmd_pec >> 8);
  cmd[3] = (uint8_t)(cmd_pec);
	LTC6811_CS_Low_2();
  LTC6811_SPI_Write_Array_2(4,cmd);
	LTC6811_CS_High_2();
}
...
void write_68_2(uint8_t total_ic , uint8_t tx_cmd[2], uint8_t data[])
{
  ...
  for (uint8_t current_ic = total_ic; current_ic > 0; current_ic--)       // executes for each LTC681x in daisy chain, this loops starts with
  {
    ...
    data_pec = (uint16_t)pec15_calc_2(BYTES_IN_REG, &data[(current_ic-1)*6]);    // calculating the PEC for each Iss configuration register data
    cmd[cmd_index] = (uint8_t)(data_pec >> 8);
    cmd[cmd_index + 1] = (uint8_t)data_pec;
    cmd_index = cmd_index + 2;
  }
  ...
  LTC6811_CS_Low_2();
  LTC6811_SPI_Write_Array_2(CMD_LEN, cmd);
  LTC6811_CS_High_2();
  free(cmd);
}
...
int8_t read_68_2( uint8_t total_ic, uint8_t tx_cmd[2], uint8_t *rx_data)
{
  ...
  LTC6811_CS_Low_2();
  LTC6811_SPI_Write_Read_2(cmd, 4, data, (BYTES_IN_REG*total_ic));         //Read the configuration data of all ICs on the daisy chain into
  LTC6811_CS_High_2();                          //rx_data[] array
  ...
  for (uint8_t current_ic = 0; current_ic < total_ic; current_ic++)       //executes for each LTC681x in the daisy chain and packs the data
  {
    ...
    received_pec = (rx_data[(current_ic*8)+6]<<8) + rx_data[(current_ic*8)+7];
    data_pec = pec15_calc_2(6, &rx_data[current_ic*8]);
    if (received_pec != data_pec)
    {
      pec_error = -1;
    }
...
/*
  Calculates  and returns the CRC15
  */
uint16_t pec15_calc_2(uint8_t len, //Number of bytes that will be used to calculate a PEC
                    uint8_t *data //Array of data that will be used to calculate  a PEC
                   )
{
  uint16_t remainder,addr;
...
uint8_t LTC681x_pladc_2()
{
  ...
  LTC6811_CS_Low_2();
  LTC6811_SPI_Write_Array_2(4,cmd);
	//adc_state = LTC6811_SPI_Read_Byte_2(0xFF);
...
uint32_t LTC681x_pollAdc_2()
{
  ...
  while ((counter<200000)&&(finished == 0))
  {
    current_time = LTC6811_SPI_Read_Byte_2(0xff);
    if (current_time>0)
    {
      finished = 1;
    }
...
// Reads the raw cell voltage register data
void LTC681x_rdcv_reg_2(uint8_t reg, //Determines which cell voltage register is read back
                      uint8_t total_ic, //the number of ICs in the
                      uint8_t *data //An array of the unparsed cell codes
                     )
{
  ...
  LTC6811_CS_Low_2();
  LTC6811_SPI_Write_Read_2(cmd,4,data,(REG_LEN*total_ic));
  LTC6811_CS_High_2();
...
int8_t parse_cells_2(uint8_t current_ic, uint8_t cell_reg, uint8_t cell_data[], uint16_t *cell_codes, uint8_t *ic_pec)
{
  ...
  received_pec = (cell_data[data_counter] << 8) | cell_data[data_counter+1]; //The received PEC for the current_ic is transmitted as the 7th and 8th
  //after the 6 cell voltage data bytes
  data_pec = pec15_calc_2(BYT_IN_REG, &cell_data[(current_ic) * NUM_RX_BYT]);
...
The function reads a single GPIO voltage register and stores thre read data
in the *data point as a byte array. This function is rarely used outside of
the LTC6811_rdaux_2() command.
*/
void LTC681x_rdaux_reg_2(uint8_t reg, //Determines which GPIO voltage register is read back
                       uint8_t total_ic, //The number of ICs in the system
                       uint8_t *data //Array of the unparsed auxiliary codes
                      )
{
...
The function reads a single stat  register and stores the read data
in the *data point as a byte array. This function is rarely used outside of
the LTC6811_rdstat_2() command.
*/
void LTC681x_rdstat_reg_2(uint8_t reg, //Determines which stat register is read back
                        uint8_t total_ic, //The number of ICs in the system
                        uint8_t *data //Array of the unparsed stat codes
                       )
{
...
after the command is sent.
*/
void LTC681x_clrsctrl_2()
{
  uint8_t cmd[2]= {0x00 , 0x18};
  cmd_68_2(cmd);
}
//Starts the Mux Decoder diagnostic self test
void LTC681x_diagn_2()
{
  uint8_t cmd[2] = {0x07 , 0x15};
  cmd_68_2(cmd);
}
...
//Reads and parses the LTC681x cell voltage registers.
uint8_t LTC681x_rdcv_2(uint8_t reg, // Controls which cell voltage register is read back.
                     uint8_t total_ic, // the number of ICs in the system
                     cell_asic ic[] // Array of the parsed cell codes
                    )
{
  ...
  if (reg == 0)
  {
    for (uint8_t cell_reg = 1; cell_reg<ic[0].ic_reg.num_cv_reg+1; cell_reg++)                   //executes once for each of the LTC6811 cell voltage registers
    {
      LTC681x_rdcv_reg_2(cell_reg, total_ic,cell_data );
      for (int current_ic = 0; current_ic<total_ic; current_ic++)
      {
        if (ic->isospi_reverse == false)
        {
        ...
        pec_error = pec_error + parse_cells_2(current_ic,cell_reg, cell_data,
                                            &ic[c_ic].cells.c_codes[0],
                                            &ic[c_ic].cells.pec_match[0]);
      }
    }
  ...
  {
    LTC681x_rdcv_reg_2(reg, total_ic,cell_data);
    ...
    for (int current_ic = 0; current_ic<total_ic; current_ic++)
    {
      ...
      pec_error = pec_error + parse_cells_2(current_ic,reg, &cell_data[8*c_ic],
                                          &ic[c_ic].cells.c_codes[0],
                                          &ic[c_ic].cells.pec_match[0]);
    }
  }
  LTC681x_check_pec_2(total_ic,CELL,ic);
  free(cell_data);
  return(pec_error);
}
...
read commands parse the data and store the gpio voltages in aux_codes variable
*/
int8_t LTC681x_rdaux_2(uint8_t reg, //Determines which GPIO voltage register is read back.
                     uint8_t total_ic,//the number of ICs in the system
                     cell_asic ic[]//A two dimensional array of the gpio voltage codes.
                    )
{
  ...
  if (reg == 0)
  {
    for (uint8_t gpio_reg = 1; gpio_reg<ic[0].ic_reg.num_gpio_reg+1; gpio_reg++)                 //executes once for each of the LTC6811 aux voltage registers
    {
      LTC681x_rdaux_reg_2(gpio_reg, total_ic,data);                 //Reads the raw auxiliary register data into the data[] array
      for (int current_ic = 0; current_ic<total_ic; current_ic++)
      {
        if (ic->isospi_reverse == false)
        {
        ...
        pec_error = parse_cells_2(current_ic,gpio_reg, data,
                                &ic[c_ic].aux.a_codes[0],
                                &ic[c_ic].aux.pec_match[0]);
  ...
  {
    LTC681x_rdaux_reg_2(reg, total_ic, data);
    ...
    for (int current_ic = 0; current_ic<total_ic; current_ic++)
    {
      ...
      pec_error = parse_cells_2(current_ic,reg, data,
                              &ic[c_ic].aux.a_codes[0],
                              &ic[c_ic].aux.pec_match[0]);
    }

  }
  LTC681x_check_pec_2(total_ic,AUX,ic);
  free(data);
  return (pec_error);
}

// Reads and parses the LTC681x stat registers.
int8_t LTC681x_rdstat_2(uint8_t reg, //Determines which Stat  register is read back.
                      uint8_t total_ic,//the number of ICs in the system
                      cell_asic ic[]
                     )

{
  ...
  if (reg == 0)
  {
    ...
    {
      data_counter = 0;
      LTC681x_rdstat_reg_2(stat_reg, total_ic,data);                            //Reads the raw statiliary register data into the data[] array
      ...
      for (uint8_t current_ic = 0 ; current_ic < total_ic; current_ic++)      // executes for every LTC6811 in the daisy chain
      {
        ...
        received_pec = (data[data_counter]<<8)+ data[data_counter+1];          //The received PEC for the current_ic is transmitted as the 7th and 8th
        //after the 6 gpio voltage data bytes
        data_pec = pec15_calc_2(BYT_IN_REG, &data[current_ic*NUM_RX_BYT]);
  ...
  else
  {
    ...
    LTC681x_rdstat_reg_2(reg, total_ic, data);
    for (int current_ic = 0 ; current_ic < total_ic; current_ic++)            // executes for every LTC6811 in the daisy chain
    {
      // current_ic is used as an IC counter
      if (ic->isospi_reverse == false)
  ...
  LTC681x_check_pec_2(total_ic,STAT,ic);
  free(data);
  return (pec_error);
}

//Write the LTC681x CFGRA
void LTC681x_wrcfg_2(uint8_t total_ic, //The number of ICs being written to
                   cell_asic ic[]
                  )
{
  uint8_t cmd[2] = {0x00 , 0x01} ;
  ...
  write_68_2(total_ic, cmd, write_buffer);
}

//Write the LTC681x CFGRB
void LTC681x_wrcfgb_2(uint8_t total_ic, //The number of ICs being written to
                    cell_asic ic[]
                   )
{
  uint8_t cmd[2] = {0x00 , 0x24} ;
...
//Read CFGA
int8_t LTC681x_rdcfg_2(uint8_t total_ic, //Number of ICs in the system
                     cell_asic ic[]
                    )
{
  uint8_t cmd[2]= {0x00 , 0x02};
  ...
  uint16_t calc_pec;
  uint8_t c_ic = 0;
  pec_error = read_68_2(total_ic, cmd, read_buffer);
  for (uint8_t current_ic = 0; current_ic<total_ic; current_ic++)
  {
    if (ic->isospi_reverse == false)
    {
    ...
    calc_pec = pec15_calc_2(6,&read_buffer[8*current_ic]);
    data_pec = read_buffer[7+(8*current_ic)] | (read_buffer[6+(8*current_ic)]<<8);
    if (calc_pec != data_pec )
    {
      ic[c_ic].config.rx_pec_match = 1;
  ...
  LTC681x_check_pec_2(total_ic,CFGRA,ic);
  return(pec_error);
}

//Reads CFGB
int8_t LTC681x_rdcfgb_2(uint8_t total_ic, //Number of ICs in the system
                      cell_asic ic[]
                     )
{
  uint8_t cmd[2]= {0x00 , 0x26};
  ...
  LTC681x_check_pec_2(total_ic,CFGRB,ic);
  return(pec_error);
}

//Looks up the result pattern for digital filter self test
uint16_t LTC681x_st_lookup_2(
  uint8_t MD, //ADC Mode
  uint8_t ST //Self Test
)
{
...
int16_t LTC681x_run_cell_adc_st_2(uint8_t adc_reg,uint8_t total_ic, cell_asic ic[])
{
  ...
  for (int self_test = 1; self_test<3; self_test++)
  {
    ...
    expected_result = LTC681x_st_lookup_2(2,self_test);
    wakeup_idle_2(total_ic);
    switch (adc_reg)
    {
      case CELL:
        wakeup_idle_2(total_ic);
        LTC681x_clrcell_2();
        LTC681x_cvst_2(2,self_test);
        LTC681x_pollAdc_2();//this isn't working
        wakeup_idle_2(total_ic);
        error = LTC681x_rdcv_2(0, total_ic,ic);
        for (int cic = 0; cic < total_ic; cic++)
        {
          for (int channel=0; channel< ic[cic].ic_reg.cell_channels; channel++)
          {
      ...
      case AUX:
        ...
        wakeup_idle_2(total_ic);
        LTC681x_clraux_2();
        LTC681x_axst_2(2,self_test);
        LTC681x_pollAdc_2();
				HAL_Delay(10);
        wakeup_idle_2(total_ic);
        LTC681x_rdaux_2(0, total_ic,ic);
        for (int cic = 0; cic < total_ic; cic++)
        {
          for (int channel=0; channel< ic[cic].ic_reg.aux_channels; channel++)
          {
      ...
      case STAT:
        wakeup_idle_2(total_ic);
        LTC681x_clrstat_2();
        LTC681x_statst_2(2,self_test);
        LTC681x_pollAdc_2();
        wakeup_idle_2(total_ic);
        error = LTC681x_rdstat_2(0,total_ic,ic);
        for (int cic = 0; cic < total_ic; cic++)
        {
          for (int channel=0; channel< ic[cic].ic_reg.stat_channels; channel++)
          {
...
int16_t LTC681x_run_adc_redundancy_st_2(uint8_t adc_mode, uint8_t adc_reg, uint8_t total_ic, cell_asic ic[])
{
  ...
  for (int self_test = 1; self_test<3; self_test++)
  {
    ...
    switch (adc_reg)
    {
      case AUX:
        LTC681x_clraux_2();
        LTC681x_adaxd_2(adc_mode,AUX_CH_ALL);
        LTC681x_pollAdc_2();
        wakeup_idle_2(total_ic);
        error = LTC681x_rdaux_2(0, total_ic,ic);
        for (int cic = 0; cic < total_ic; cic++)
        {
          for (int channel=0; channel< ic[cic].ic_reg.aux_channels; channel++)
          {
      ...
      case STAT:
        LTC681x_clrstat_2();
        LTC681x_adstatd_2(adc_mode,STAT_CH_ALL);
        LTC681x_pollAdc_2();
        wakeup_idle_2(total_ic);
        error = LTC681x_rdstat_2(0,total_ic,ic);
        for (int cic = 0; cic < total_ic; cic++)
...
void LTC681x_run_openwire_2(uint8_t total_ic, cell_asic ic[])
{
  ...
  wakeup_sleep_2(total_ic);
  LTC681x_adow_2(MD_7KHZ_3KHZ,PULL_UP_CURRENT);
  LTC681x_pollAdc_2();
  wakeup_idle_2(total_ic);
  LTC681x_adow_2(MD_7KHZ_3KHZ,PULL_UP_CURRENT);
  LTC681x_pollAdc_2();
  wakeup_idle_2(total_ic);
  error = LTC681x_rdcv_2(0, total_ic,pullUp_cell_codes);

  wakeup_idle_2(total_ic);
  LTC681x_adow_2(MD_7KHZ_3KHZ,PULL_DOWN_CURRENT);
  LTC681x_pollAdc_2();
  wakeup_idle_2(total_ic);
  LTC681x_adow_2(MD_7KHZ_3KHZ,PULL_DOWN_CURRENT);
  LTC681x_pollAdc_2();
  wakeup_idle_2(total_ic);
  error = LTC681x_rdcv_2(0, total_ic,pullDwn_cell_codes);
...
uint16_t LTC681x_run_adc_overlap_2(uint8_t total_ic, cell_asic ic[])
{
  ...
  int16_t failure_neg_limit = -20;
  wakeup_idle_2(total_ic);
  LTC681x_adol_2(MD_7KHZ_3KHZ,DCP_DISABLED);
  LTC681x_pollAdc_2();
  wakeup_idle_2(total_ic);
  error = LTC681x_rdcv_2(0, total_ic,ic);
  for (int cic = 0; cic<total_ic; cic++)
...
void LTC681x_init_cfg_2(uint8_t total_ic, cell_asic ic[])
{
  ...
  for (uint8_t current_ic = 0; current_ic<total_ic; current_ic++)
  {
    ...
    LTC681x_set_cfgr_2(current_ic ,ic,REFON,ADCOPT,gpioBits,dccBits);
...
{
  LTC681x_set_cfgr_refon_2(nIC,ic,refon);
  LTC681x_set_cfgr_adcopt_2(nIC,ic,adcopt);
  LTC681x_set_cfgr_gpio_2(nIC,ic,gpio);
  LTC681x_set_cfgr_dis_2(nIC,ic,dcc);
}
...
//Writes the comm register
void LTC681x_wrcomm_2(uint8_t total_ic, //The number of ICs being written to
                    cell_asic ic[]
                   )
{
  uint8_t cmd[2]= {0x07 , 0x21};
...
Reads COMM registers of a LTC6811 daisy chain
*/
int8_t LTC681x_rdcomm_2(uint8_t total_ic, //Number of ICs in the system
                      cell_asic ic[]
                     )
{
  uint8_t cmd[2]= {0x07 , 0x22};
...
// Writes the pwm register
void LTC681x_wrpwm_2(uint8_t total_ic,
                   uint8_t pwmReg,
                   cell_asic ic[]
                  )
{
...
Reads pwm registers of a LTC6811 daisy chain
*/
int8_t LTC681x_rdpwm_2(uint8_t total_ic, //Number of ICs in the system
                     uint8_t pwmReg,
                     cell_asic ic[]
                    )
{
